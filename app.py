...
# --- 페이지 기본 설정 ---
st.set_page_config(page_title="아기 돼지 삼형제: 튼튼한 집 짓기", layout="wide", page_icon="🐷")
...
st.markdown('<div class="sub-header">아기 돼지 삼형제의 후손들과 함께 튼튼한 집을 짓는 AI를 만들어보아요!</div>', unsafe_allow_html=True)
...
# 모델 1: 단순 선형 회귀
with col_m1:
    st.subheader("1️⃣ 단순한 방법 (선형 회귀)")
    st.markdown("직선 하나로만 관계를 찾으려고 해요.")
    if st.button("단순 모델 학습시키기"):
        lr = LinearRegression()
...
# 모델 2: 다항 회귀
with col_m2:
    st.subheader("2️⃣ 똑똑한 방법 (다항 회귀)")
    st.markdown("직선이 아니라 곡선으로 복잡한 관계를 찾아요!")
    
    if st.button("똑똑한 모델 학습시키기", type="primary"):
        # 특성 공학
...
if st.session_state['model_poly'] is None:
    st.warning("☝️ 먼저 위에서 '똑똑한 모델 학습시키기' 버튼을 눌러주세요!")
else:
